...
`My Website > CMS` is now CMS Studio.

CMS Studio is a single website content and layout workspace with:
...
The underlying models, migrations, seeders, resolvers, and storefront runtime services remain in place because they power the structured runtime layer and future Studio workflows.

CMS Studio is not a free-form page builder. It edits safe website content and layout slots only. Product detail pages, category listing pages, cart, checkout, search results, customer account pages, order pages, and transactional commerce flows remain controlled by commerce and theme logic.

Header Builder is implemented with structured fields and a preview panel. It saves into existing header JSON storage internally.
