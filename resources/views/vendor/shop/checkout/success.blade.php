            ...
            <div class="flex flex-wrap items-center justify-center gap-3">
                @auth('customer')
                    <a href="{{ route('shop.customers.account.orders.view', $order->id) }}">
                        <div class="w-max cursor-pointer rounded-2xl border border-navyBlue px-11 py-3 text-center text-base font-medium text-navyBlue max-md:rounded-lg max-md:px-6 max-md:py-1.5">
                            See Order
                        </div>
                    </a>
                @endauth
