                        ...
                        <div class="flex items-center gap-3">
                            <template v-for="(option, index) in attribute.options">
                                <template v-if="option.id">
                                    ...
                                    <label 
                                        class="group relative flex cursor-pointer items-center justify-center overflow-hidden rounded-md border bg-white p-1 font-medium uppercase text-gray-900 hover:bg-gray-50 shrink-0"
                                        style="display: flex; align-items: center; justify-content: center; width: 72px !important; height: 72px !important; min-width: 72px !important; min-height: 72px !important; aspect-ratio: 1 / 1 !important; flex: 0 0 72px !important;"
                                    ...
                                    >
                                        ...
                                        <img
                                            :src="option.swatch_value ? option.swatch_value : ''"
                                            :title="option.label"
                                            class="h-full w-full"
                                            style="object-fit: contain; object-position: center; background-color: #fff;"
                                        />
                                    </label>
        ...
        <script type="module">
            ...
            app.component('v-product-configurable-options', {
                ...
                methods: {
                    ...
                    reloadImages () {
                        const parentGalleryImages = JSON.parse(JSON.stringify(this.galleryImages ?? []));
                        const galleryRef = this.$parent?.$parent?.$refs?.gallery;

                        if (! parentGalleryImages.length || ! galleryRef) {
                            return;
                        }

                        const colorAttribute = this.childAttributes.find(attribute => attribute.swatch_type === 'image' || attribute.swatch_type === 'color');
                        const selectedColorId = colorAttribute?.selectedValue;
                        const selectedColorIndex = colorAttribute?.options?.findIndex(option => option.id == selectedColorId) ?? -1;
                        const normalizedIndex = selectedColorIndex > 0 ? selectedColorIndex - 1 : 0;

                        galleryImages.splice(0, galleryImages.length, ...parentGalleryImages);

                        galleryRef.media.images = [...parentGalleryImages];
                        galleryRef.activeIndex = normalizedIndex;

                        const selectedMedia = parentGalleryImages[normalizedIndex] ?? parentGalleryImages[0];

                        if (selectedMedia?.type === 'videos') {
                            galleryRef.baseFile.type = 'video';
                            galleryRef.baseFile.path = selectedMedia.video_url;
                        } else {
                            galleryRef.baseFile.type = 'image';
                            galleryRef.baseFile.path = selectedMedia.large_image_url;
                        }
                    },
                }
            });
