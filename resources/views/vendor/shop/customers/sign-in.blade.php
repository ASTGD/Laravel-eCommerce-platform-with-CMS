@php
    $canResendVerification = (bool) request()->cookie('enable-resend') && request()->cookie('email-for-resend');
    $verificationEmail = request()->cookie('email-for-resend');
    $requestedRedirect = request()->query('redirect_to');
@endphp
...
>
    ...
    <div class="container mt-20 max-1180:px-5 max-md:mt-12">
        ...
        <div class="m-auto w-full max-w-[870px] rounded-xl border border-zinc-200 p-16 px-[90px] max-md:px-8 max-md:py-8 max-sm:border-none max-sm:p-0">
            ...
            @if ($canResendVerification && $verificationEmail)
                <div class="mt-8 rounded-xl border border-amber-200 bg-amber-50 px-6 py-5">
                    <p class="text-base font-semibold text-amber-900">
                        @lang('shop::app.customers.login-form.verify-first')
                    </p>

                    <p class="mt-2 break-all text-sm text-amber-800">
                        {{ $verificationEmail }}
                    </p>

                    <div class="mt-4">
                        <x-shop::form :action="route('shop.customers.resend.verification')">
                            <input
                                type="hidden"
                                name="email"
                            ...
                            <button
                                type="submit"
                                class="secondary-button m-0 rounded-xl px-6 py-3 text-sm"
                            >
