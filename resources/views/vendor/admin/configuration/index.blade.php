    ...
    <div class="grid gap-y-8">
        @foreach (system_config()->getItems() as $item)
            @php
                ...
                if ($item->getKey() === 'sales') {
                    $children = $children->reject(fn ($child) => in_array($child->getKey(), [
                        'sales.carriers',
            ...
            <div>
                ...
                <div class="box-shadow max-1580:grid-cols-3 mt-2 grid grid-cols-4 flex-wrap justify-between gap-12 rounded bg-white p-4 dark:bg-gray-900 max-xl:grid-cols-2 max-sm:grid-cols-1">
                    @foreach ($children as $key => $child)
                        @php
                            $childKey = \Illuminate\Support\Str::afterLast($child->getKey(), '.');
                            $childName = $child->getKey() === 'sales.shipping'
                                ? 'Shipping'
                                : $child->getName();
                            $childInfo = $child->getKey() === 'sales.shipping'
                                ? 'Manage origin, checkout methods, courier operations, and shipment notifications in one place.'
                                : $child->getInfo();
                        @endphp
