...
<section class="gadget-section gadget-latest" aria-labelledby="gadget-latest-title">
    <div class="gadget-container">
        ...
        @php
            $displayLatestProducts = count($products) > 0 ? $products : array_fill(0, 8, [
                'id' => 0,
            ...
            $items = is_array($displayLatestProducts) ? $displayLatestProducts : $displayLatestProducts->toArray();
            
            // If fewer than 6 items, duplicate them to force scrolling
            if (count($items) > 0 && count($items) < 6) {
                $displayLatestProducts = array_merge($items, $items);
            } else {
                $displayLatestProducts = $items;
            }
        @endphp

        <v-gadget-product-carousel :item-count="{{ count($displayLatestProducts) }}">
                ...
                <div class="carousel-container" @mouseenter="stopAutoplay" @mouseleave="startAutoplay">
                    ...
                    <div class="carousel-track-wrap">
                        <div class="carousel-track" :style="[translateStyle, { '--items-per-view': itemsPerView }]">
                            @foreach ($displayLatestProducts as $product)
