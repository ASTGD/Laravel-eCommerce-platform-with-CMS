...
@php($productImages = $product->images->values())
<section class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200">
    ...
    <div class="mt-4 grid gap-4 md:grid-cols-2">
        @forelse ($gallery as $index => $image)
            @php($productImage = $productImages->get($index))
            @php($imageUrl = $productImage?->path ? Storage::url($productImage->path) : ($image['original_image_url'] ?? $image['large_image_url'] ?? $image['medium_image_url'] ?? $image['small_image_url'] ?? ''))
            <div class="overflow-hidden rounded-[1.5rem] bg-slate-100 p-3">
                <img src="{{ $imageUrl }}" alt="{{ $product->name }}" class="h-full w-full rounded-[1rem] object-cover">
            </div>
        @empty
