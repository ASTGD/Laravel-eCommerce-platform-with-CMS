...
<section class="mx-auto max-w-6xl px-6 py-14">
    ...
    <div class="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        @forelse ($items as $item)
            @php($productUrl = ! empty($item->url_key) ? route('shop.product_or_category.index', $item->url_key) : '#')

            <a
                href="{{ $productUrl }}"
                class="block rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
                aria-label="{{ $item->name ?? $item->sku ?? 'Product' }}"
            >
