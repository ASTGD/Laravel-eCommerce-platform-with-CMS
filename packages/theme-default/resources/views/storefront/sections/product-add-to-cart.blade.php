...
@php($hasVariants = \Webkul\Product\Helpers\ProductType::hasVariants($product->type))
...
<section class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200">
    ...
    @if ($isDirectCartType)
        <form
            ...
            data-add-to-cart-form="section-{{ $section['id'] }}"
        >
            @csrf
    ...
    @elseif ($hasVariants)
        <form
            ...
            data-configurable-cart-form="product-{{ $product->id }}"
        >
            @csrf
            <input type="hidden" name="product_id" value="{{ $product->id }}">
            <input type="hidden" name="selected_configurable_option" value="" data-configurable-selection-input>

            <div class="flex flex-wrap items-center gap-3">
                <input type="number" min="1" name="quantity" value="{{ $section['settings']['default_quantity'] ?? 1 }}" class="w-24 rounded-lg border border-slate-300 px-3 py-2 text-sm">
                <button
                    type="submit"
                    class="rounded-full bg-slate-900 px-5 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:bg-slate-400"
                    data-configurable-submit-button
                    disabled
                >
                    {{ $section['settings']['button_label'] ?? 'Add to cart' }}
                </button>
        ...
        <p class="mt-3 text-sm text-slate-500" data-add-to-cart-message="section-{{ $section['id'] }}">Choose size and color first, then add the selected variant to the cart.</p>

        <script>
            window.addEventListener('DOMContentLoaded', () => {
                const form = document.querySelector('[data-configurable-cart-form="product-{{ $product->id }}"]');
                const message = document.querySelector('[data-add-to-cart-message="section-{{ $section['id'] }}"]');
                ...
                form.addEventListener('submit', async (event) => {
                    ...
                    const selectedInput = form.querySelector('[data-configurable-selection-input]');

                    if (! selectedInput?.value) {
                        message.textContent = 'Select size and color first.';
                        return;
                    }
    ...
    @else
        <p class="mt-4 text-sm text-slate-600">This product type does not need additional cart options.</p>
    @endif
</section>
